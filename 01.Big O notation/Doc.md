...
func sum_odds_evens(nums []int) string {
    odds, evens := 0, 0
    for _, num := range nums {
        if num % 2 == 0 {
            evens += num
...
func count_frequency(nums []int) map[int]int {
...
func generate_pairs(arr []int) []struct{first, second int} {
    pairs := []struct{first, second int}{}
    for _, i := range arr {
        for _, j := range arr {
            pairs = append(pairs, struct{first, second int}{i, j})
        }
    }
    return pairs
}
